...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, N):
        ...
        #self.norm = nn.BatchNorm1d(layer.size, eps=1e-4, momentum=0.85)
...
class SublayerConnection(nn.Module):
    ...
    def __init__(self, size, dropout):
        ...
        #self.norm = nn.BatchNorm1d(size, eps=1e-4, momentum=0.85)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        #self.linears = clones(BinaryLinear(d_model, d_model), 4)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, d_ff, dropout=0.1):
        ...
        #self.w_1 = BinaryLinear(d_model, d_ff)
        #self.w_2 = BinaryLinear(d_ff, d_model)
...
def run_epoch(data_iter, model, loss_compute):
    ...
    for i, batch in enumerate(data_iter):
        ...
        #for p in model.decoder.layers.parameters():
        #    p.data.clamp_(-1.0, 1.0)
        total_loss += loss
        ...
        if i % 50 == 1:
            ...
            print("Epoch Step: %d Loss: %f Tokens per Sec: %f" %
                    (i, loss / batch.ntokens, tokens.item() / elapsed))
            start = time.time()
            tokens = 0
        gc.collect()
        #mem_report()
